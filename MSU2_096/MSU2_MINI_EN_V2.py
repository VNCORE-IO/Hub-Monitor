...
def Get_Photo_Path1():  # Get file path
    ...
    photo_path1 = tk.filedialog.askopenfilename(title="Select File", filetypes=[('Image file', '*.jpg'), ('Image file', '*.jpeg'), ('Image file', '*.png'), ('Image file', '*.bmp')])
...
def Get_Photo_Path3():  # Get file path
    ...
    photo_path3 = tk.filedialog.askopenfilename(title="Select File", filetypes=[('Image file', '*.jpg'), ('Image file', '*.jpeg'), ('Image file', '*.png'), ('Image file', '*.bmp')])
    Label5.config(text=photo_path3[-20:])


def Get_Photo_Path4():  # Get file path
    global photo_path4, Label6
    photo_path4 = tk.filedialog.askopenfilename(title="Select File", filetypes=[('Image file', '*.jpg'), ('Image file', '*.jpeg'), ('Image file', '*.png'), ('Image file', '*.bmp')])
...
def Print_MSN_Data():
    ...
    for i in range(0, num):  # Print all data
        data_str = data_str + 'Index: ' + str(i) + '    Name: ' + str(My_MSN_Data[i].name) + '    Unit: ' + str(My_MSN_Data[i].unit)
...
def Write_Flash_Page_fast(Page_add, data_w, Page_num):  # Without erasing, directly write 256B data to specified Flash page
    # First complete data transfer
    hex_use = b''
    for i in range(0, 64):  # 256 bytes divided into 64 commands
        ...
        if (G_screnn0_OK == 0 or G_screnn1_OK == 0):
            ...
            if (size_mode == 0):  # Horizontal fill
                if (size_PC.width >= size_PC.height * 2):
                    ...
                    im1 = im1.crop((0, (160 * size_PC.height // size_PC.width - 80) // 2, 160, (160 * size_PC.height // size_PC.width - 80) // 2 + 80))  # Center crop
            elif (size_mode == 1):  # Vertical fill
                if (size_PC.height * 2 >= size_PC.width):
                    im1 = im.resize((size_USE_X1, size_USE_Y1))  # Scale
                else:
                    im1 = im.resize((80 * size_PC.width // size_PC.height, 80))  # Scale
                    im1 = im1.crop(((80 * size_PC.width // size_PC.height - 160) // 2, 0, (80 * size_PC.width // size_PC.height - 160) // 2 + 160, 80))  # Center crop
            elif (size_mode == 2):  # Stretch fill
                im1 = im.resize((size_USE_X1, size_USE_Y1))  # Scale
            ...
            for y in range(0, size_USE_Y1):
                for x in range(0, size_USE_X1):
                    hex_16RGB.append(((im2[x, y][0] >> 3) << 11) | ((im2[x, y][1] >> 2) << 5) | (im2[x, y][2] >> 3))  # First directly add 16bit array
...
def UI_Page():  # Display GUI
    ...
    root.geometry(str(Show_W) + "x" + str(Show_H) + "+" + str(Show_X) + "+" + str(Show_Y))  # Main window size and position on display
    # Create buttons
    btn1 = tk.Button(root, text="Previous Page", height=1, width=8)
    btn1.place(x=400, y=275, anchor="w")  # Set position and alignment
    btn1.config(command=Page_UP)  # Connect button trigger event

    btn2 = tk.Button(root, text="Next Page", height=1, width=8)
    btn2.place(x=400, y=325, anchor="w")  # Set position and alignment
    btn2.config(command=Page_Down)  # Connect button trigger event

    btn3 = tk.Button(root, text="Select Background Image", height=1, width=12)
    ...
    btn4 = tk.Button(root, text="Select Flash Firmware", height=1, width=12)
    ...
    btn11 = tk.Button(root, text="Select Animated Image", height=1, width=12)
    ...
    # Create sliders
    s1 = Scale(root, from_=0, to=31, resolution=1, troughcolor='Red', orient=HORIZONTAL)  # orient=HORIZONTAL horizontal, default vertical
    s1.place(x=150, y=25, anchor="w")  # W.get()
    s1.set(31)
    s2 = Scale(root, from_=0, to=63, resolution=1, troughcolor='Green', orient=HORIZONTAL)  # orient=HORIZONTAL horizontal, default vertical
    s2.place(x=250, y=25, anchor="w")  # W.get() can get slider value
    s2.set(0)
    s3 = Scale(root, from_=0, to=31, resolution=1, troughcolor='Blue', orient=HORIZONTAL)  # orient=HORIZONTAL horizontal, default vertical
    s3.place(x=350, y=25, anchor="w")  # W.get() can get slider value
    s3.set(0)
    # Create labels
    Label1 = tk.Label(root, text="Device Not Connected", bg="Red")
    Label1.place(x=25, y=25, anchor="w")  # Set position and alignment

    Label2 = tk.Label(root, bg="Red", width=2)
    Label2.place(x=460, y=25, anchor="w")  # Set position and alignment
    ...
    if len(port_list) == 0:
        ...
        for i in range(0, len(port_list)):
            try:  # Attempt to open serial port
                ...
                print(port_list[i].name + ' cannot be opened, please check if it is being used by another program')  # Display MSN device count
                # ser.close() # Close serial port to prevent failure on next open
                time.sleep(0.1)
                continue  # Continue to next loop
            time.sleep(0.25)  # In theory, MSN device sends " MSN01" every 100ms, should receive at least once within 250ms
            recv = SER_Read()
            if (recv == 0):
            ...
            if (len(recv) > 5):  # Only parse when receiving 6 or more characters
                for n in range(0, len(recv) - 5):  # Parse encoding character by character
                    if (ord(recv[n]) == 0):  # Parse when current byte is 0
                        if ((recv[n + 1] == 'M') and (recv[n + 2] == 'S') and (recv[n + 3] == 'N')):  # Ensure it is an MSN device
                            if ((recv[n + 4] >= '0') and (recv[n + 4] <= '9') and (recv[n + 5] >= '0') and (recv[n + 5] <= '9')):  # Ensure version number is ASCII digit
                                My_MSN_Device.append(MSN_Device((port_list[i].name), (ord(recv[4]) - 48) * 10 + (ord(recv[5]) - 48)))  # Register MSN device
                                ...
                                time.sleep(0.25)  # In theory, MSN device sends " MSN01" every 100ms, should receive at least once within 250ms
                                recv = SER_Read().decode("gbk")  # Get serial data
                                if ((ord(recv[0]) == 0) and (recv[1] == 'M') and (recv[2] == 'S') and (recv[3] == 'N') and (recv[4] == 'C') and (recv[5] == 'N')):  # Ensure it is an MSN device
                                    print('MSN device ' + str(len(My_MSN_Device)) + ' — ' + port_list[i].name + ' connection completed')  # Display MSN device count
                                else:
                                    print('MSN device ' + str(len(My_MSN_Device)) + ' cannot connect, please check connection')  # Display MSN device count
                                break  # Exit current for loop
...
print("This device has " + str(psutil.cpu_count(logical=False)) + " cores and " + str(psutil.cpu_count()) + " logical processors")
